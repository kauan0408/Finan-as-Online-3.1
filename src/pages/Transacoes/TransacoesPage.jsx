  ...
  export default function TransacoesPage() {
    ...
    const lerGastosDaFoto = async (evento) => {
      ...
      try {
        ...
        setItensLote((atuais) => [...atuais, ...novos]);
        mostrarMensagem(`✅ ${novos.length} lançamento(s) encontrado(s). Confira os campos.`);
      } catch (erro) {
    ...
    const salvarItensLote = () => {
      const invalidos = itensLote.filter(
    ...
    const finalizarPorSilencio = (textoInformado) => {
      ...
      setItensLote((atuais) => [...atuais, ...validos]);
      setReviewText(finalText);
      setTextoVoz(finalText);
      mostrarMensagem(`✅ ${validos.length} lançamento(s) criado(s). Confira e salve.`);
    };
